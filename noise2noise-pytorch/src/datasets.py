...
class NoisyDataset(AbstractDataset):
    ...
    def _corrupt(self, img):
        ...
        if self.noise_type in ['gaussian', 'poisson', 'adv']:
    ...
    def __getitem__(self, index):
        ...
        im_name = ''
        for k in range(len(img_path), 1, -1):
            if(img_path[k-1] != '/'):
                im_name = im_name + img_path[k-1]
            else:
                break
...
#         new_img = Image.open(new_path+im_name).convert('RGB')
  ...
  #################################################################################test 주석###################################################        
        ...
        if self.clean_targets:
            ...
            #target = tvF.to_tensor(self._corrupt(new_img))
            target = tvF.to_tensor(new_img)
